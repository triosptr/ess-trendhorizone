...
module.exports = async function handler(req, res) {
  ...
  const photoUrl = String((body && body.foto_keluar_url) || '').trim();
  const now = new Date();
  const yyyy = now.getFullYear();
  const mm = String(now.getMonth() + 1).padStart(2, '0');
  const dd = String(now.getDate()).padStart(2, '0');
  const tanggal = String((body && body.tanggal) || (yyyy + '-' + mm + '-' + dd)).trim();
  const jamKeluar = String((body && body.jam_keluar) || now.toTimeString().slice(0, 8)).trim();
  ...
  if (!inserted.ok) {
    return json(res, 500, {
      ok: false,
      message: 'Gagal menyimpan check-out ke Supabase.',
  ...
  return json(res, 200, {
    ok: true,
    message: 'Check-out berhasil disimpan ke Supabase.',
    data: inserted.data
  });
};
